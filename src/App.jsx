...
export default function App() {
  ...
  const handleSubmit = () => {
    ...
    if (window.Telegram?.WebApp) {
      ...
      alert('Тестовый режим: ' + JSON.stringify(payload, null, 2));
    }
  };
  ...
  return (
    <div className="container">
      ...
      <textarea
        placeholder="Соцсети (через запятую)"
        value={data.socials}
        onChange={e => setData({...data, socials: e.target.value})}
        rows="2"
      />

      <textarea
        placeholder="Услуги (по одной на строку, можно добавить длительность через |, например: Консультация | 45)"
        value={data.services}
      ...
      <select
        ...
        onChange={e => setData({...data, design_style: e.target.value})}
      >
      ...
      <label>
        <input
          type="checkbox"
          checked={data.needs_hosting}
          onChange={e => setData({...data, needs_hosting: e.target.checked})}
        />
      ...
      <div style={{ marginTop: '16px' }}>
        ...
        {['calendar', 'payments', 'ai_chat', 'analytics'].map(feat => (
          <label key={feat} style={{ display: 'block', margin: '6px 0' }}>
            <input
              type="checkbox"
              checked={data.extra_features.includes(feat)}
